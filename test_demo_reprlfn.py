    ...
    if mode == "test":
        path = [
            (
                ...
                os.path.join(data_dir, f"DIV2K_test_HR/{i:04}x4.png")
        ...
        path = [
            (
                ...
                # os.path.join(data_dir, f"DIV2K_valid_HR/{i:04}x4.png")
...
def main(args):
    ...
    for k, v in results.items():
        ...
        if args.include_test:
            ...
            test_time = f"{v['test_ave_runtime']:3.2f}"
            # ave_time = f"{(v['valid_ave_runtime'] + v['test_ave_runtime']) / 2:3.2f}"
            ave_time = f"{(v['test_ave_runtime']) :3.2f}"
            mem = f"{v['test_memory']:2.2f}"
            # s += fmt.format(k, val_psnr, test_psnr, val_time, test_time, ave_time, num_param, flops, mem)
            s += fmt.format(k, test_psnr, test_time, ave_time, num_param, flops, mem)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--include_test", default=True, action="store_true", help="Inference on the DIV2K test set")
